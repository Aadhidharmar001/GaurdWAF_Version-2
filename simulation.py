...
def run_simulation():
    ...
    r1 = client.post(f"{BASE_URL}/proxy/tool", json={
    ...
    r2 = client.post(f"{BASE_URL}/proxy/tool", json={
        "agent_id": "support-agent",
        "session_id": session_1,
        "tool": "send_email",
        "parameters": {
    ...
    r3 = client.post(f"{BASE_URL}/proxy/tool", json={
        "agent_id": "support-agent",
    ...
    r4 = client.post(f"{BASE_URL}/proxy/tool", json={
    ...
    client.post(f"{BASE_URL}/proxy/tool", json={
        "agent_id": "support-agent", "session_id": session_4,
        "tool": "verify_recipient", "parameters": {"recipient": "bob@aivar.com"}
    })
    for i in range(1, 5):
        res = client.post(f"{BASE_URL}/proxy/tool", json={
            "agent_id": "support-agent", "session_id": session_4,
            "tool": "send_email", "parameters": {"recipient": "bob@aivar.com", "subject": f"Ping {i}"}
        }).json()
    ...
    r5 = client.post(f"{BASE_URL}/proxy/tool", json={
        "agent_id": "query-agent", "session_id": session_5,
        "tool": "execute_query", "parameters": {"query": "SELECT * FROM Users; DROP TABLE Users;"}
    }).json()
    ...
    client.post(f"{BASE_URL}/proxy/tool", json={
        "agent_id": "support-agent", "session_id": session_6,
        "tool": "verify_recipient", "parameters": {"recipient": "client@external.com"}
    })
    r6 = client.post(f"{BASE_URL}/proxy/tool", json={
        "agent_id": "support-agent", "session_id": session_6,
        "tool": "send_email", "parameters": {"recipient": "client@external.com", "subject": "External Quote"}
    }).json()
