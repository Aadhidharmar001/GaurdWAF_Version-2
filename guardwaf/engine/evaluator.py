...
from guardwaf.core.models import ActionIntent, PolicyConfig, RuleResult, PendingAction, ActionState
...
class ActionEvaluator:
    ...
    def evaluate(self, intent: ActionIntent) -> RuleResult:
            ...
            if rule.tool == intent.tool_name:
                if rule.condition_param and rule.greater_than is not None:
                    ...
                    if isinstance(val, (int, float)) and val > rule.greater_than:
                        ...
                        tenant_id = intent.session_context.tenant_id if (intent.session_context and intent.session_context.tenant_id) else "default"
        ...
        return RuleResult(status="allowed", outcome="Action Permitted by Policy.", matched_rule=None, risk_score=0.0, risk_level="LOW")
