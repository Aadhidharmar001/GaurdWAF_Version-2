...
class MCPGatewayProxy:
    ...
    """
    def __init__(self, waf: GuardWAF, downstream_handler: Optional[Callable[[MCPToolRequest], MCPToolResponse]] = None, backend_server: Optional[Any] = None):
        self.waf = waf
    ...
    def handle_tool_request(self, request: MCPToolRequest) -> MCPToolResponse:
        ...
        try:
            ...
            return MCPToolResponse(
                id=request.id,
                error={"code": -32001, "message": f"GuardWAF Security Error: {str(err)}"}
            )

        if not allowed:
            return MCPToolResponse(
                id=request.id,
                error={"code": -32002, "message": reason or f"MCP tool '{request.tool_name}' blocked by GuardWAF policy."}
            )
        ...
        if forwarded_digest != evaluated_digest:
            return MCPToolResponse(
                id=request.id,
                error={"code": -32003, "message": "CRITICAL: Parameter mutation detected between authorization and forwarding."}
            )
        ...
        return MCPToolResponse(
            id=request.id,
            result={"status": "ALLOWED_AND_EXECUTED", "tool_name": request.tool_name, "grant_id": grant.grant_id if grant else None}
        )

    def invoke_tool(self, tool_name: str, arguments: dict, tenant_id: str = "default", agent_id: str = "mcp_agent") -> dict:
        req = MCPToolRequest(
            id="req_mcp_1",
            tool_name=tool_name,
            arguments=arguments,
