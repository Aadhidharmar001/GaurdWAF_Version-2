...
from guardwaf.core.models import PolicyConfig, PendingAction, ActionState, SessionContext
...
from guardwaf.sdk.context import session, verified_session, get_current_session
...
from guardwaf.sdk.context import session, verified_session, get_current_session, get_current_execution_context
...
class GuardWAF:
    ...
    def deny_pending_action(self, pending_action_id: str, approver_id: str = "admin", reason: str = "Denied by human reviewer") -> PendingAction:
    ...
    async def resume(
        ...
        if action.status == ActionState.DENIED:
            raise GuardWAFSecurityError(f"Action '{pending_action_id}' was DENIED and cannot be resumed.", tool_name=action.tool_name)
        ...
        if action.status in [ActionState.EXECUTED, ActionState.EXECUTING]:
            raise GuardWAFAlreadyExecutedError(f"Action '{pending_action_id}' has already been executed.", tool_name=action.tool_name, pending_action_id=pending_action_id)
        if action.status != ActionState.APPROVED:
            raise GuardWAFInvalidStateTransitionError(f"Action '{pending_action_id}' is not in APPROVED state (current: {action.status.value}).")
        ...
        if not token_valid:
            raise GuardWAFSecurityError(f"Invalid, expired, or forged approval token for action '{pending_action_id}'.", tool_name=action.tool_name)
        ...
        if current_param_digest != action.parameter_digest:
            raise GuardWAFSecurityError(f"Parameter digest mismatch for action '{pending_action_id}'. Parameters were tampered with.", tool_name=action.tool_name)
        ...
        if not func:
            ...
            raise GuardWAFConfigurationError(f"No execution body registered for tool '{action.tool_name}'. Decorate the function with @protect().")
        ...
        try:
            ...
            self.telemetry.emit(TelemetryEvent(
                ...
                parameter_digest=action.parameter_digest,
                status="failed",
                outcome=f"Tool execution failed during resume: {str(e)}",
                pending_action_id=pending_action_id
            ))
            raise e
