...
from guardwaf.control_plane.events.events import SignedRevocationEvent, EventSignatureVerifier

class RevocationTransport(ABC):
    @abstractmethod
    def broadcast_event(self, event: SignedRevocationEvent) -> None: pass
    @abstractmethod
    def subscribe(self, callback: Callable[[SignedRevocationEvent], None]) -> None: pass
