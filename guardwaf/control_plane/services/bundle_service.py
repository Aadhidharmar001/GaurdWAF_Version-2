    ...
    def compile_and_sign_bundle(
        ...
        if agent.status == AgentStatus.REVOKED:
            if self.audit_service:
                self.audit_service.record_event("BUNDLE_REJECTED", tenant_id, "system", "agent", agent_id, {"reason": "Agent REVOKED"})
            raise GuardWAFSecurityError(f"Agent '{agent_id}' has been REVOKED. Refusing to issue policy bundle.", tool_name="bundle_compiler")

        if agent.tenant_id != tenant_id:
            raise GuardWAFSecurityError(f"Tenant mismatch: Agent '{agent_id}' belongs to tenant '{agent.tenant_id}', not '{tenant_id}'.", tool_name="bundle_compiler")
        ...
        if self.audit_service:
            self.audit_service.record_event("BUNDLE_ISSUED", tenant_id, "system", "bundle", bundle_id, {"agent_id": agent_id})
