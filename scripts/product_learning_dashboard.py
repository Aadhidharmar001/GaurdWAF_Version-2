...
def main():
    ...
    print(
        f"   • Automated Unit Tests:          {eng_evidence['automated_tests_passing']} / 130+ PASS"
    )
    print(f"   • Package Release Build:         {eng_evidence['package_build_status']}")
    print(
        f"   • Clean Virtualenv Install:      {eng_evidence['clean_installation_status']}"
    )
    print(
        f"   • Framework Integrations:        {', '.join(int_evidence['flagship_demos'])} (INTERNALLY TESTED)"
    )
    print("   • Engineering Completion State:  ✅ PASSED & VERIFIED")

    print("\n2. REAL EXTERNAL BETA SIGNALS (Sample Size Rules Enforced)")
    print(
        f"   • External Developers Sample (n):{ext_evidence['external_developers_count']} (Rule: n < 5 = Qualitative Signal Only)"
    )
    print(
        f"   • Successful Integrations:       {ext_evidence['external_successful_integrations']}"
    )
    print("   • Failed Integrations:           NOT YET MEASURED")
    print(
        "   • Production Deployment Intent:  UNKNOWN (Would Deploy Production: Yes/Maybe/No)"
    )
    print(
        "   • Primary Adoption Barrier:      UNKNOWN (Security / Framework / Complexity / Control Plane)"
    )
