...
def main():
    ...
    print(
        f"   • Clean Environment Install:     {eng_evidence['clean_installation_status']}"
    )
    print(f"   • CI/CD Security Pipeline:       {eng_evidence['ci_cd_status']}")

    print("\n2. INTERNAL VALIDATION (Maintainer Demos & Integration Harnesses)")
    print(
        f"   • Internal Demos Verified:       {', '.join(int_evidence['flagship_demos'])}"
    )
    print(
        f"   • Maintainer Validation Status:  {int_evidence['internal_maintainer_validation']}"
    )

    print("\n3. OPT-IN ANONYMOUS TELEMETRY (Privacy-Respecting)")
    print(
        f"   • Status:                        {'ENABLED' if opt_evidence['enabled'] else 'OFF BY DEFAULT (Opt-In Only)'}"
    )
    print(
        f"   • Recorded Usage Events:         {opt_evidence['recorded_events_count']}"
    )
    print(f"   • Privacy Boundary:              {opt_evidence['privacy_boundary']}")

    print("\n4. EXTERNAL DEVELOPER EVIDENCE (Real Third-Party Developers)")
    print(
        f"   • External Developers Count:     {ext_evidence['external_developers_count']}"
    )
    print(
        f"   • External Integrations:         {ext_evidence['external_successful_integrations']}"
    )
    print(
        f"   • Public Beta Feedback:          {ext_evidence['external_feedback_reports']}"
    )
