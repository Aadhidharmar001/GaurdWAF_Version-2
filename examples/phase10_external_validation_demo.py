...
def main():
    ...
    run_cmd(
        ...
        "Scenario 2: Verifying Clean Environment Package Build & Installation",
    )

    # Scenario 3: Starter Project Execution (SIMULATED DEVELOPER)
    run_cmd(
        [sys.executable, "examples/external_beta/main.py"],
        "Scenario 3: SIMULATED DEVELOPER Executing Starter Kit Project",
    )
    ...
    run_cmd(
        [sys.executable, "examples/phase10_real_world/customer_support/main.py"],
        "Scenario 5: Executing Customer Support Agent (Read, Refund, Block)",
    )
    ...
    run_cmd(
        [sys.executable, "examples/phase10_real_world/financial_agent/main.py"],
        "Scenarios 7-10: Financial Agent (HITL Suspension, Approval, Resume, Replay Block)",
    )

    # Scenario 11: Framework Integration (LangGraph)
    run_cmd(
        [sys.executable, "examples/phase10_real_world/langgraph_agent/main.py"],
        "Scenario 11: Executing LangGraph Workflow State Graph Node Governance",
    )

    # Scenario 12: MCP Gateway Integration
    run_cmd(
        [sys.executable, "examples/phase10_real_world/mcp_agent/main.py"],
        "Scenario 12: Executing MCP Gateway Proxy Tool Discovery & Parameter Mutation Defense",
    )
    ...
    run_cmd(
        [sys.executable, "scripts/import_beta_feedback.py"],
        "Scenario 15: Running Beta Feedback Intake & Schema Validation",
    )
    ...
    run_cmd(
        [sys.executable, "scripts/reproduce_beta_issue.py"],
        "Scenario 17: Executing External Issue Reproduction CLI Tool",
    )

    # Scenario 18: Product Learning Dashboard
    run_cmd(
        [sys.executable, "scripts/product_learning_dashboard.py"],
        "Scenario 18: Executing Phase 10 Product-Market Learning Dashboard",
    )
