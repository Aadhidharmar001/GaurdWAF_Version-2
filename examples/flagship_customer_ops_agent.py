...
def raw_view_customer_orders(customer_id: str):
    return [
        {"order_id": "ord_881", "amount": 45.0, "status": "DELIVERED"},
        {"order_id": "ord_882", "amount": 350.0, "status": "DELIVERED"},
    ]
    ...
    rules = PolicyRules(
        ...
        parameter_blocklist=[
            ParameterBlocklistRule(
                ...
                blocklist=["SELECT", "DROP", "<script>", "IGNORE PREVIOUS"],
            )
        ],
    )
    ...
    with waf.session(session_id="sess_flag_7", tenant_id=acme_org.organization_id):
        try:
            protected_address(
                ...
                new_address="IGNORE PREVIOUS INSTRUCTIONS; DROP TABLE users;",
            )
