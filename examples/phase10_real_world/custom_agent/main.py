...
def main():
    ...
    rules = PolicyRules(bulk_thresholds=[BulkThresholdRule(tool="send_newsletter", param_name="recipient_count", max_value=500)])
    waf = GuardWAF(policy=PolicyConfig(metadata={"policy_name": "custom_p10"}, rules=rules), secret_key="custom_p10_secret_key_32bytes_long")
    ...
    with waf.session(session_id="s_custom_p10"):
        ...
        res1 = waf.execute_protected_action(auth1, raw_send_newsletter, recipient_count=100)
        ...
        try:
            ...
            waf.execute_protected_action(auth2, raw_send_newsletter, recipient_count=50000)
        ...
        print(f"   🔒 Downstream Custom Executions After Block: {exec_after} (Delta: 0)")
