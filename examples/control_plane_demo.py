...
def run_control_plane_demo():
    ...
    print(
        f"   ✅ Version 1 Created: ID='{ver1.version_id}', SHA-256 Digest={ver1.content_digest[:20]}..."
    )
    ...
    policy_service.assign_policy(
        tenant_id=tenant_id, policy_id=pol_rec.policy_id, agent_id=agent_id
    )
    ...
    bundle_v1 = bundle_service.compile_and_sign_bundle(
        tenant_id=tenant_id, agent_id=agent_id
    )
    print(
        f"   ✅ Signed Bundle Issued: ID='{bundle_v1.bundle_id}', KeyID='{bundle_v1.key_id}'"
    )
    ...
    print(
        "   ✅ SDK PolicyClient Cryptographically Verified and Applied Signed Bundle v1"
    )

    # Initialize GuardWAF SDK with PolicyClient
    waf = GuardWAF(policy_client=policy_client)
    alice_principal = VerifiedPrincipal(
        principal_id="usr_alice", tenant_id=tenant_id, subject="alice"
    )
    ...
    with waf.verified_session(principal=alice_principal, agent=support_agent):
        ...
        print(
            f"      - Local Policy Evaluation Latency: {eval_time_ms} ms (Verified < 1ms target!)"
        )
    ...
    v2_rules = {
        "sequences": [{"tool": "process_refund", "requires": ["lookup_customer"]}],
        "bulk_thresholds": [
            {"tool": "process_refund", "param_name": "amount", "max_value": 100}
        ],
    }
    ...
    with waf.verified_session(principal=alice_principal, agent=support_agent):
        ...
        print(
            "   ✅ SUCCESS: Executed $50.00 refund using Last Known Good Policy during Control Plane outage!"
        )

    # --- Scenario 11: Attacker Sends Tampered Policy Bundle ---
    print("\n▶ SCENARIO 11: Attacker sends tampered policy bundle to SDK...")
    valid_bundle = bundle_service.compile_and_sign_bundle(
        tenant_id=tenant_id, agent_id=agent_id
    )
    ...
    print(
        "   ✅ GUARDWAF REJECTED TAMPERED BUNDLE: Active LKG policy remained unchanged!"
    )
    ...
    print("\n" + "=" * 85)
    print(
        "✅ PHASE 3B DEMO COMPLETE: Control Plane, Signed Bundles & SDK Governance Verified!"
    )
    print("=" * 85)
