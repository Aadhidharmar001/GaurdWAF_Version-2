...
from guardwaf.mcp.wrapper import MCPToolWrapper, secure_server
...
rules = PolicyRules(
    bulk_thresholds=[BulkThresholdRule(tool="process_refund", param_name="amount", max_value=500)]
)
...
def run_universal_demo():
    ...
    try:
        langchain_refund.run(customer_id="cust_bob", amount=600.00, tenant_id=tenant_id, agent_id=agent_id)
    ...
    try:
        crewai_refund._run(customer_id="cust_charlie", amount=600.00, tenant_id=tenant_id, agent_id=agent_id)
