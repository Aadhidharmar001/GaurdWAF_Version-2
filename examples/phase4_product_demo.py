...
def run_phase4_demo():
    ...
    sarah = org_service.create_user(
        email="sarah@acme.com", display_name="Sarah (Owner)"
    )
    john = org_service.create_user(
        email="john@acme.com", display_name="John (Developer)"
    )
    maya = org_service.create_user(
        email="maya@acme.com", display_name="Maya (Security Admin)"
    )
    ...
    print(
        f"   ✅ Organization Created: Name='{acme_org.name}', ID='{acme_org.organization_id}'"
    )
    print(
        f"   ✅ Members Added: Owner={sarah.display_name}, Developer={john.display_name}, SecurityAdmin={maya.display_name}"
    )
    ...
    print(
        f"   ✅ Agent Registered: ID='{agent_rec.agent_id}', Tenant='{agent_rec.tenant_id}', Status={agent_rec.status.value}"
    )
    ...
    v1_rules = {
        "bulk_thresholds": [
            {"tool": "process_refund", "param_name": "amount", "max_value": 500}
        ],
        "hitl_rules": [
            {"tool": "process_refund", "param_name": "amount", "threshold": 500}
        ],
    }
    ver1 = policy_service.create_version(
        pol_rec.policy_id, rules=v1_rules, actor_id=maya.user_id
    )
    policy_service.publish_and_activate(
        pol_rec.policy_id, version_number=1, actor_id=maya.user_id
    )
    ...
    print(
        f"   ✅ Policy Version 1 Activated: SHA-256 Digest={ver1.content_digest[:20]}..."
    )
    ...
    cred_res = cred_service.issue_credential(
        organization_id=acme_org.organization_id, agent_id=agent_id
    )
    ...
    print(
        f"   ✅ API Key Authenticated Successfully for Agent '{verified_cred.agent_id}'!"
    )
    ...
    policy_obj = PolicyConfig(
        metadata={"policy_name": "refund_policy"}, rules=rules_obj
    )
    ...
    try:
        RBACManager.check_permission(
            role=Role.DEVELOPER, required_action=ACTION_PUBLISH_POLICY
        )
    ...
    agent_service.revoke_agent(
        agent_id=agent_id, actor_id=maya.user_id, reason="Suspicious activity detected"
    )
    ...
    print(
        f"   ✅ Incident Logged: ID='{inc.incident_id}', Severity={inc.severity.value}, Status={inc.status.value}"
    )

    print("\n" + "=" * 85)
    print(
        "✅ PHASE 4 DEMO COMPLETE: Multi-Tenant Enterprise Platform & Production Control Plane Verified!"
    )
    print("=" * 85)
