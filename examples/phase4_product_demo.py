...
from guardwaf.core.models import PolicyConfig, PolicyRules, BulkThresholdRule, HITLRule, ActionState
...
from guardwaf.control_plane.auth.rbac import RBACManager, ACTION_PUBLISH_POLICY
...
def run_phase4_demo():
    ...
    policy_service.assign_policy(tenant_id=acme_org.organization_id, policy_id=pol_rec.policy_id, agent_id=agent_id)
    ...
    try:
        org_service.verify_tenant_isolation(requesting_org_id=beta_org.organization_id, target_resource_org_id=acme_org.organization_id)
    ...
    print(f"   ✅ Dashboard Stats: Active Agents={stats['active_agents']}, Revoked Agents={stats['revoked_agents']}, Total Events={stats['total_audit_events']}")
