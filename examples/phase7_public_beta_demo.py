...
def main():
    ...
    run_cmd(
        ...
        "1. Building PyPI Release Artifacts (sdist & wheel)",
    )

    # 2. Clean Environment Package Install & Verification
    run_cmd(
        [sys.executable, "scripts/verify_package_install.py"],
        "2. Verifying Clean Virtual Environment Package Installation",
    )

    # 3. 60-Second Quickstart
    run_cmd(
        [sys.executable, "examples/60_second_quickstart/main.py"],
        "3. Running 60-Second Quickstart Project",
    )

    # 4-8. Framework Integration Examples
    run_cmd(
        [sys.executable, "examples/integrations/langchain_agent/main.py"],
        "4. Executing LangChain Framework Integration",
    )
    run_cmd(
        [sys.executable, "examples/integrations/langgraph_agent/main.py"],
        "5. Executing LangGraph State Graph Integration",
    )
    run_cmd(
        [sys.executable, "examples/integrations/crewai_agent/main.py"],
        "6. Executing CrewAI Task Integration",
    )
    run_cmd(
        [sys.executable, "examples/integrations/mcp_server/main.py"],
        "7. Executing MCP Gateway Server Integration",
    )
    run_cmd(
        [sys.executable, "examples/integrations/custom_agent/main.py"],
        "8. Executing Custom Agent Loop Integration",
    )

    # 9. Hero Feature Playground
    run_cmd(
        [sys.executable, "examples/playground/main.py"],
        "9. Executing Hero Feature GuardWAF Playground",
    )
    ...
    run_cmd(
        [
            ...
            "-m",
            "pytest",
            "tests/test_simulated_developer_onboarding.py",
        ],
